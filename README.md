...
`upstreamtop` is a live, terminal-native view of every backend an nginx reverse proxy is `proxy_pass`-ing to, ranked by requests per second, broken down per route, with per-backend status mix and average latency.

> [!TIP]
> **No nginx config, no log parsing, no restart.** Two `tcx` programs attach to the kernel's egress and ingress paths and read the plain-HTTP upstream traffic as it flows past. nginx never knows it's there, and it keeps working across `nginx -s reload`.

## Quick start

```sh
curl -fsSL https://yeet.cx/?utm_source=github&utm_medium=readme&utm_campaign=upstreamtop | sh
...
- `--ignore <port|ip:port>` — hide an endpoint. Useful when the load generator runs on the same host as nginx.
...
The "recent requests" feed at the bottom is a tail of the last few proxied requests with method, path, and backend — for spotting a one-off curl that hit an unexpected endpoint.
...
| `on_response` | `tcx/ingress` | When a TCP segment begins with `HTTP/`, parse the 3-digit status code at the fixed offset, look up the request by 4-tuple, attribute status + latency back to its `(backend, route)` bucket, and free the inflight entry. |
...
- **Plain HTTP only.** If nginx talks to backends over TLS (`proxy_pass https://...`), the bytes on the wire are encrypted and there's nothing line-shaped to parse. The displacement there is a `uprobe` on `SSL_write` paired with `SSL_get_fd` to recover the backend — a different script.
...
The simulated backends have distinct profiles so the dashboard is worth looking at: `:9001`/`:9002` are a fast round-robin pool, `:9003` (`/checkout`) is slow (60–160ms) and returns 5xx around 12% of the time.
...
The BPF program declares `SEC("license") = "GPL"` in [`upstreamtop.bpf.c`](upstreamtop.bpf.c), which is required for the kernel helpers it uses. The JS side has no separate license declaration in the source. (extrapolated, grounding 2 — review)

---

Built with [yeet](https://yeet.cx/?utm_source=github&utm_medium=readme&utm_campaign=upstreamtop), a JS runtime for writing eBPF programs on Linux machines. Join us on [discord](https://discord.gg/dYZu9PjKB?utm_source=github&utm_medium=readme&utm_campaign=upstreamtop).
